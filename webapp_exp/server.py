...
def initialize_serial(port='COM3', baudrate=9600, timeout=1):
    global ser
    try:
...
def plan_trajectory(start_angles, end_angles, steps=10):
    trajectory = []
    for step in range(steps):
        ...
        trajectory.append(intermediate_angles)
    return trajectory
...
def handle_animation():
    ...
    for target_angles in steps:
        if not ser or not ser.is_open:
            logging.error("Serial port not available during animation.")
            break
        trajectory = plan_trajectory(current_angles, target_angles)
        for angles in trajectory:
            for i, angle in enumerate(angles):
                move_servo(i, angle)
            current_angles = angles
            socketio.emit('update_angles', {'angles': current_angles})
            socketio.sleep(0.1)  # Adjust as needed for smoothness
